...
func insertCategory(db *sql.DB) {
	...
	repoCategory := repository.RepoCategory{
		Category: model.Category{
			Name: model.Request.FormCategory.Name,
		},
	}
	_, err = repository.CreateRepo(&repoCategory, tx)
	if err != nil {
		handler.BadRequest(err.Error())
		return
	}
	response, err := repoCategory.FindById(tx)
	if err != nil {
		handler.BadRequest(err.Error())
		return
	}
